...
def select_ico_file():
    fileico = filedialog.askopenfilename(filetypes=(("Image File", "*.png *.jpeg"), ("All files", "*.*")))
...
def build():
    ...
    main = [f'python3 -m nuitka --product-name="{name}" --output-filename={name} --file-version={version} --company-name="{author}" --standalone "{pyfile}" --macos-create-app-bundle']
